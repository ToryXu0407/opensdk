...
import com.nascent.ecrp.opensdk.openplatformrequest.auth.GetBrandListRequest;
...
public class GetBrandListTest {
    ...
    private static void getResponse(GetBrandListRequest request, DefaultOpenPlatformClient defaultOpenPlatformClient) {
        try {
            GetBrandListResponse response = defaultOpenPlatformClient.execute(request);
            System.out.println(response.getResult().get(0).getBrandId());
